...
namespace FastColoredTextBoxNS
{
        ...
        public static SyntaxDescriptor ParseXmlDescription(XmlDocument doc)
        {
            ...
            if (brackets != null)
            {
                if (brackets.Attributes["left"] == null || brackets.Attributes["right"] == null ||
                    brackets.Attributes["left"].Value == string.Empty || brackets.Attributes["right"].Value == string.Empty)
                {
                    desc.leftBracket = '\x0';
    ...
    brackets.Attributes["left2"].Value == string.Empty || brackets.Attributes["right2"].Value == string.Empty)
                {
        ...
        void InitHTMLRegex()
        {
            ...
            HTMLAttrRegex = new Regex(@"(?<range>\S+?)='[^']*'|(?<range>\S+)=string.Empty[^string.Empty]*string.Empty|(?<range>\S+)=\S+", RegexOptions.Compiled);
            HTMLAttrValRegex = new Regex(@"\S+?=(?<range>'[^']*')|\S+=(?<range>string.Empty[^string.Empty]*string.Empty)|\S+=(?<range>\S+)", RegexOptions.Compiled);
        }
