...
- (id)init {
	...
	[self netStatsForInterval:sampleInterval];
...
- (int) getNetuse
{
    NSDictionary* netdict = [self netStatsForInterval:sampleInterval];
    double deltasum = 0;
    ...
    deltasum /= sampleInterval; // convert to per-sec
    deltasum /= 1000; // conver to kB
    return deltasum;
}
